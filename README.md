...
docs/
  ...
  demo-page.html  a self-contained render of that board -- the page script is
                  stripped, so it calls nothing when opened (clone and open it;
                  GitHub serves .html in docs/ as source, not as a page)
  logo-*.svg      light and dark header art
```

`requirements-lock.txt` at the top level pins the versions `docs/output.txt` was
captured on.
